...
LOST_LINE_TOLERANCE = 10  # 允许连续丢线帧数（约等于 0.5 秒）
    ...
    try:
        while True:
            ...
            if line_center is not None:
                ...
                print(f"循线模式: 前进 {x_speed:.2f} m/s, 转向 {z_speed:.1f}°/s")

                # 更新状态
                last_x_speed, last_z_speed = x_speed, z_speed
                lost_line_counter = 0
            else:
                # 丢线情况
                lost_line_counter += 1
                if lost_line_counter <= LOST_LINE_TOLERANCE:
                    # 短暂丢线时保持上一帧方向（过断线）
                    ep_chassis.drive_speed(x=last_x_speed, y=0, z=-last_z_speed, timeout=0.1)
                    print(f"断线补偿: 保持前进 {last_x_speed:.2f} m/s, 转向 {last_z_speed:.1f}°/s")
                else:
                    # 长时间丢线则停止
                    ep_chassis.drive_speed(x=0, y=0, z=0, timeout=0.1)
                    print("长时间未检测到蓝线，停止运动")
