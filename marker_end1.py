...
class MarkerCaptureManager:
    def __init__(self,
                 ...
                 *,
                 ...
                 only_near: bool = True,
                 kp_yaw: float = 0.35,  # ↓ 降增益，减小来回抖
                 max_yaw_dps: float = 45.0,  # ↓ 降最大转速
                 min_yaw_dps: float = 5.0,
                 deadband_dps: float = 3.0,
                 dir_yaw: float = +1.0,             # 方向反了可以改成 -1
                 smooth_alpha: float = 0.45,  # 轨迹 EMA 平滑
                 ...
                 # —— 起步抖动抑制 / 捕获稳定化 ——
                 anti_osc_flip_window: float = 0.6,  # 在多少秒内统计“转向翻转”次数
                 anti_osc_max_flips: int = 3,  # 超过几次判定为“抖动”
                 anti_osc_freeze: float = 0.5,  # 触发后冻结云台多长时间（秒）
                 settle_frames: int = 4  # 连续多少帧“对准”后才允许拍照
                 ):
        ...
        self._ao_flip_times: list = []   # 记录翻转时间戳
        self._ao_freeze_until     = 0.0
        # 捕获稳定帧计数
        self.settle_frames        = int(settle_frames)
        self._settle_cnt          = 0
        # 发送 yaw 时做一个简单的斜坡，避免突变
        self._yaw_cur_dps         = 0.0
        self._yaw_slew_dps        = 12.0   # 每次调用允许的最大变化(°/s)，可按需要加大/减小
    ...
    def _choose_target(self, stable: List[MarkerInfo]) -> Optional[MarkerInfo]:
        ...
        # 2) 已有锁时：只要锁定目标还在“stable”里，就无条件继续用它；
        #    即使出现了第二个目标，也不允许切，除非超过最小占用时长 + 宽限失败。
        if self._locked_id is not None:
            ...
            if locked_obj is not None:
                ...
                if (now - self._lock_since) < self.lock_min_dwell:
                    return locked_obj
                # 到了最小占用时长也优先继续用锁定目标（稳定第一）
                return locked_obj

            # 锁定的目标暂时不在候选里（丢失），在宽限时间内不换目标
            if (now - self._last_lock_ts) <= self.lost_grace_sec:
                return None
            # 宽限超时：释放锁，允许重新选择
            self._locked_id = None

        # 3) 没有锁：选择“first_ts 最早（第一个出现）”
        if not candidates:
            return None
    ...
    def _send_yaw(self, yaw_speed_norm: float, max_speed_norm: float = 0.25) -> None:
        ...
        # —— 斜坡限幅（把 yaw 变化量限制到每次 _yaw_slew_dps 以内）——
        target = yaw_dps_raw
        if target > self._yaw_cur_dps:
            self._yaw_cur_dps = min(self._yaw_cur_dps + self._yaw_slew_dps, target)
        else:
