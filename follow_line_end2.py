...
from marker_end1 import MarkerCaptureManager      # 工具类 step 内只驱动云台 yaw
...
def read_newest(cam, timeout=0.3):
    # 尝试 newest，失败则普通模式；都失败就返回 None（不抛异常）
    try:
...
if __name__ == '__main__':
    ...
    marker_mgr = MarkerCaptureManager(
        ...
        center_tol=0.07, width_thresh_norm=0.08,  # 稍放宽对准与拍照阈值
        kp_yaw=0.35, max_yaw_dps=45.0,  # 与 A-1 保持一致
        deadband_dps=3.0,
                ...
                if line_center is not None:
                    ...
                    if not search_guard:
                        ...
                        if not searching:
                            guard_drop_ts = None
                            searching = True
                            sweep_dir = -sweep_dir if sweep_dir != 0 else +1  # 进入搜索时翻转方向，防止总向一边
                            seg_end_ts = time.time() + SEARCH_SEG_TIME
                            found_cnt = 0
                            print("[SEARCH] 丢线 → 原地旋转扫描")
                            motion.send(0.0, sweep_dir * SEARCH_YAW_DPS)
